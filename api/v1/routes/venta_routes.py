...
def create_venta(
    ...
    Permite registrar ventas de productos con múltiples ítems y actualiza
...
@router.get(
    ...
    description="Busca una venta específica usando su ID único."
)
def get_venta_by_id(
    id: int,
    service: VentaService = Depends(get_venta_service)
...
@router.get(
    ...
    description="Busca todas las ventas registradas en una fecha específica."
)
...
def delete_venta(
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="No se puede eliminar la venta por dependencias o "
                   "error interno"
        )
